...
const double H[3][3][3] = {{{2*Jpm+2*Jpp,0,0},{0,2*Jpm-2*Jpp,Jzp},{0,Jzp,Jzz}}, {{2*Jpm-Jpp,-sqrt(3)*Jpp,-sqrt(3)/2*Jzp},{-sqrt(3)*Jpp,2*Jpm+2*Jpp,-Jzp/2},{-sqrt(3)/2*Jzp,-Jzp/2,Jzz}}, {{2*Jpm-Jpp,+sqrt(3)*Jpp,+sqrt(3)/2*Jzp},{+sqrt(3)*Jpp,2*Jpm+2*Jpp,-Jzp/2},{+sqrt(3)/2*Jzp,-Jzp/2,Jzz}}};
...
double energy()
{
	...
	for (inow = 0; inow < L; inow++) for (jnow = 0; jnow < L; jnow++)
	{
		...
		nb[2] = s[pre(inow)][nex(jnow)], 	nb[3] = s[pre(inow)][jnow];
		nb[4] = s[inow][pre(jnow)], 		nb[5] = s[nex(inow)][pre(jnow)];

		ds = s[inow][jnow];
		E += dot(0,ds,nb[0]) + dot(1,ds,nb[2]) + dot(2,ds,nb[4]); //+ dot(0,ds,nb[3]) + dot(1,ds,nb[5]) + dot(2,ds,nb[1]);
...
{
	int i, j, inow, jnow;
	for (i = 0; i < L; i++)	for (j = 0; j < L; j++) gen(&s[i][j]);
	e_av = 0;
	m_av = 0;
	e_q_av = 0;
	m_q_av = 0;
